...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('Error fetching active alert from Supabase:', error);
    }
    ...
    return NextResponse.json({ success: false, activeAlert: null, error: err.message });
  }
}
  ...
  try {
    ...
    if (action === 'TRIGGER') {
      ...
      const formattedAlert: EmergencyAlert = {
        ...data,
        code_details: EMERGENCY_CODES[data.code_id] || EMERGENCY_CODES.code_blue,
        patient_details: data.patient_details || patient,
      };
    ...
    if (action === 'RESPOND') {
      ...
      return NextResponse.json({ success: true, responder: data });
    }
